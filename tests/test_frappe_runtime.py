...
from frappe_runtime import handlers as handlers_mod
from frappe_runtime.auth import Session
from frappe_runtime.config import DEFAULT_WORKER_THREADS, RealtimeConfig, get_config
...
class TestServerApp(unittest.IsolatedAsyncioTestCase):
	...
	def setUp(self):
		try:
			from frappe_runtime import server as server_mod
	...
	def test_discovery_gets_the_sites_path_from_the_config(self):
		...
		with patch.object(self.server_mod, "wire", lambda *a, **k: None):
			with patch(
				"frappe_runtime.registry.discover_app_handlers",
