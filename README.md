...
With no file, `bench restore` reads the `backup-access` secret, finds the newest backup folder, downloads the database and the site-config backup, and restores them — **creating the site first if it does not exist**, so a fresh clone needs nothing but `git submodule update --init` (the shell never checks the apps out for you — see [Local apps](#local-apps)), `direnv allow`, `devenv up`, `bench restore`.
...
-   a **git submodule** — `.gitmodules` names it, `bench-update --pull` moves it, `nix build` fetches it. `bench-get-app` makes these. Nothing else moves one: entering the shell only reports a registered submodule with no checkout — a fresh clone's, say — and the command that checks it out, since doing it for you on every `nix develop` is how an app removed by hand came straight back, re-cloned.
