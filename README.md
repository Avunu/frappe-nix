...
-   reproducible **node\_modules** for every app and nested frontend, from committed lockfiles no hash is ever computed for (`importNpmLock`);
...
├── node-locks/               # generated: a package-lock.json per app and nested frontend — commit it
└── sites/
...
└── nix/
    ├── uv.lock                     # committed lock — drives the Nix Python env
    └── node-locks/                 # committed package-lock.json per app and nested frontend
```
...
| what is committed | pyproject.toml, uv.lock, node-locks/, sites/ | flake.nix, nix/uv.lock, nix/node-locks/ |
...
nix run .#relock             # re-resolve, rewriting nix/uv.lock + nix/node-locks/
```
...
```sh
nix run .#relock             # nix/node-locks/ is needed for this, not for the shell
...
Afterwards, run `bench-update --node-locks` inside the dev shell before the first `nix build` — `node-locks/` is not generated by the migration (it needs the network and npm).
        ...
        perSystem =
          { pkgs, ... }:
          {
            frappe-nix = {
              ...
              # Before the first `nix build`: `bench-update --node-locks` in the
              # dev shell, and commit node-locks/. See Node lockfiles.
            };
          };
      }
    );
...
| app.lockDir | str | "nix" | Where the generated-but-committed uv.lock and node-locks/ live, relative to the repo root. |
...
| nodeOverrides | attrs of attrs | {} | Per node target (app, or "app/subdir"): extra derivationArgs for the importNpmLock.buildNodeModules that builds its node_modules — npmRebuildFlags = "" runs lifecycle scripts, npmDeps replaces the fetch. See Node lockfiles. |
| nodeNestedFrontendExcludes | list of str | [] | Nested frontends ("app/subdir") to leave out: no lock, no node_modules, no assets, and the parent's build script that drives them is dropped. See Node lockfiles. |
...
-   **`bench-update --pull` re-locks.** It regenerates `node-locks/` for every app whose manifests moved, and runs `uv lock` when any app's `pyproject.toml` moved, so the pull that causes the drift also resolves it. Commit `uv.lock` and `node-locks/` with the submodule bumps.
...
| bench-update [--pull\|--migrate\|--build\|--node-locks] | Submodule-aware replacement for bench update. --pull fetches each submodule's .gitmodules branch from the remote that carries its declared URL (origin is often a developer's fork), refuses to discard local commits, skips local apps and reports stray repos; then regenerates sites/apps.json for the new pins, node-locks/ for the apps whose manifests moved, and re-locks the workspace (uv lock) when a pyproject.toml did. --node-locks regenerates node-locks/ for every app and nested frontend. In app mode, --migrate and --build only. |
...
| yarn add / yarn install | bench-update --node-locks → node-locks/<app>/package-lock.json → importNpmLock |
...
Each app with a `package.json`, and each nested frontend under it (`erpnext/banking`, `hrms/frontend`, `commit/dashboard` — an immediate subdirectory with its own `package.json`), is a **node target**. For every target the bench commits a directory under **`node-locks/`** (`nix/node-locks/` in app mode):

```
node-locks/<app>/package-lock.json    # what the build installs from — every entry carries its integrity
node-locks/<app>/package.json         # the manifest npm resolved against (yarn `resolutions` → npm `overrides`)
node-locks/<app>/source.json          # sha256 of the app's package.json / yarn.lock — the staleness stamp
node-locks/<app>/<subdir>/…           # nested frontends, the same three files
```

They are generated, never edited by hand. **`bench-update` keeps them current**: `bench-update` / `bench-update --pull` regenerates the targets whose manifests moved during the pull (the stamp makes the others free), `bench-update --node-locks` regenerates all of them, `update-deps` regenerates after its `yarn install`s, and in app mode `nix run .#relock` does the same over the pinned inputs. Generation needs the network and npm (the dev shell's `frappe-nix.nodejs`); `nix build` and CI only read what is committed. Evaluation warns about a target with no lock (the package then carries no `node_modules` for it, and `bench build` fails on it if it has a build script) and about a lock older than its manifests.

No hash is ever computed. The build is `pkgs.importNpmLock.buildNodeModules` per target: each package is fetched by the integrity the lock already carries, a git dependency by its commit (`builtins.fetchGit`, at evaluation time — a cold cache needs the repository reachable the first time anything forces the package, `nix flake check --no-build` included). An upstream `yarn.lock` bump costs one lock regeneration, and refetches only what moved.

**With or without a `yarn.lock`.** npm resolves the lock with `npm install --package-lock-only`. When the app ships a `yarn.lock`, npm pins to it — the exact versions and integrity it lists, the tree upstream tested — so the lock is that `yarn.lock` in npm's dialect. When it does not, npm resolves from `package.json`; commit the lock and the app is pinned from then on. Both run with `--legacy-peer-deps` (yarn v1 never auto-installed peers) and the build installs with the same flag; lifecycle scripts are not run in the sandbox (`--ignore-scripts`, as before — every native piece a Frappe frontend needs is a platform package or a prebuilt binary). A target that needs them: `nodeOverrides."<app>/<subdir>".npmRebuildFlags = ""`. `nodeOverrides.<target>` is the `derivationArgs` of that target's `buildNodeModules`, so `postPatch`, `nativeBuildInputs` and an `npmDeps` of your own (`pkgs.importNpmLock { … fetcherOpts … }`, to fetch a git dependency differently) all go there; the flag attributes are strings, because the derivation uses structured attrs and a list would reach npm as its first element.

In the bench tree each target's `node_modules` is a real directory of links into the store, not a link to the store's directory: Vite creates `node_modules/.vite-temp` while bundling an ESM `vite.config`, and only a permission error is tolerated — the sandbox's read-only store answers `EROFS`. `bench build` is unchanged (`yarn run production`, then `yarn build` in every app with a build script); the dev shell is unchanged too — its `node_modules` is a plain online `yarn install`, as upstream tooling expects.

**Nested frontends** are discovered one level deep: a subdirectory the app tracks as a git submodule (`hrms/frappe-ui`) is a project of its own and skipped; `nodeNestedFrontendExcludes = [ "app/subdir" ]` leaves a frontend out entirely — no lock, no `node_modules`, no assets — for one whose build is broken upstream or whose routes the deployment never serves. (The case this knob was born for, an upstream `yarn.lock` that could not resolve offline, no longer arises: npm resolves such gaps from the registry when the lock is generated.) Because a nested frontend is usually built by its *parent* app's `build` script (`cd banking && yarn build`) and frappe's esbuild runs every app's `build` script with no opt-out, excluding one also drops the parent's `build`/`postinstall` scripts that name it from the bench tree's `package.json`; the build log says so per script. Forking the app is the alternative, and the trade is maintenance.

**Migrating a bench** from `node-offline-hashes.json`: drop `nodeOfflineHashes` (it is an error now) and any yarn-era `nodeOverrides` (`yarnFlags` did nothing already), run `bench-update --node-locks`, commit `node-locks/`, delete `node-offline-hashes.json`. In app mode: `nix run .#relock`, commit `nix/node-locks/`, delete `nix/node-offline-hashes.json`.
